  ...
  try {
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      if (data[i].categories !== undefined) {
        pool.query({
          text: 'INSERT INTO submissions SELECT nextval($1), $2, TO_TIMESTAMP($3, $4), $5, $6 WHERE NOT EXISTS (SELECT * FROM submissions WHERE name = $7 AND lineid = $8);',
          values: ['submissions_submissionid_seq', data[i].categories[0].split('_')[0], deadline.replace('T', ' '), 'YYYY/MM/DD HH24:MI:SS', data[i].summary, lineID, data[i].summary, lineID],
        });
        // userslecturesの更新
        // 知らない組み合わせを得たら更新する
        pool.query({
